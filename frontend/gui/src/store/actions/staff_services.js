...
export const getFilteredList = (filters) => {
  return async (dispatch) => {
    ...
    if (token === undefined) {
      ...
      await axios
        .post(`http://127.0.0.1:8000/api/staff/graduate-list/`, filters)
        .then((response) => {
          dispatch(transactionTypes.D_Success({ tableData: response.data }));
        })
        .catch((error) => {
...
export const setApproval = (enrollment, message, type) => {
  return async (dispatch) => {
    ...
    if (token === undefined) {
      ...
      await axios
        .post(`http://127.0.0.1:8000/api/staff/graduate-data/${enrollment}/`, {
          message: message,
          type: type,
        })
        .then((response) => {
          dispatch(transactionTypes.D_Success({ approval: response.data }));
        })
        .catch((error) => {
          dispatch(transactionTypes.D_Fail(error));
        });
...
export const getGraduate = (enrollment) => {
  return async (dispatch) => {
    ...
    if (token === undefined) {
      ...
      await axios
        .get(`http://127.0.0.1:8000/api/staff/graduate-data/${enrollment}/`)
        .then((response) => {
          dispatch(
            transactionTypes.D_Success({
...
export const getDocumentsDetails = () => {
  return async (dispatch) => {
    ...
    if (token === undefined) {
      ...
      await axios
        .get(`http://127.0.0.1:8000/api/process/2/documents/descriptions/`)
        .then((response) => {
          dispatch(transactionTypes.D_Success({ documents: response.data }));
        })
        .catch((error) => {
          dispatch(transactionTypes.D_Fail(error));
        });
