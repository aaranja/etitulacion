...
export const processGetTitulationTypes = () => {
  ...
  return (dispatch) => {
    ...
    if (token === undefined) {
      ...
      axios
        .get(`http://127.0.0.1:8000/api/process/4/titulation/types/`)
        .then((response) => {
...
const getFile = async (dispatch, id, token) => {
  ...
  await axios({
    ...
    Authorization: `Token ${token}`,
    url: `http://127.0.0.1:8000/staff/services/settings/signature/${id}/`,
    responseType: "blob",
  })
    .then((response) => {
      let settingsfiles = {};
  ...
  return async (dispatch) => {
    ...
    if (token === undefined) {
      ...
      await axios({
        method: "get",
        Authorization: `Token ${token}`,
        url: `http://127.0.0.1:8000/staff/services/settings/signature/preview-cni/`,
