...
class InstituteDetails extends Component {
  ...
  onSave = (values) => {
    // get id to save
    values["id"] =
      this.props.instituteData.id !== undefined
        ? this.props.instituteData.id
        : null;
        ...
        >
          <Descriptions size="small" column={1}>
            <Descriptions.Item label={<b>Información</b>}>
              Los datos del instituto se utilizan para generar la constancia de
              no inconvenientes y la certificación de acta de examen
              profesional.
            </Descriptions.Item>
          </Descriptions>
        </PageHeader>
        ...
        >
          <fieldset disabled={!this.state.editing}>
            ...
            >
              ...
              >
                <Input
                  placeholder="Nombre del jefe(a) de servicios escolares"
                  size="large"
                  bordered={editing}
                />
              </Form.Item>
              <Form.Item key={updateIndex()} name="director" label="Director">
                <Input
...
const mapStateToProps = (state) => {
  ...
  if (payload !== null) {
    if (payload["institute"] !== undefined) {
      ...
      loadingData =
        institute.status === "loading" && institute.action === "get";
